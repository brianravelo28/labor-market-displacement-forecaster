...
- **Small-occupation noise** — occupations with employment in the tens/hundreds show large YoY % swings (OEWS sampling error dominates at that scale). Visible in the "top contracting/growing" tables; treat single-occupation extremes with caution, especially at the industry-sector cross-tab level where samples are even smaller.
